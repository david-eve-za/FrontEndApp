...
(function (factory) {
    ...
    Mapael.prototype = {
        ...
        drawLink: function (id, xa, ya, xb, yb, elemOptions) {
            ...
            // distribute(c,d) = distribute(a,b) (=abDist)
            var abDist = Math.sqrt((xb - xa) * (xb - xa) + (yb - ya) * (yb - ya));

            // Solution for equation distribute(cd) = sqrt((xd - xc)² + (yd - yc)²)
            // distribute(c,d)² = (xd - xc)² + (yd - yc)²
            // We assume that distribute(c,d) = distribute(a,b)
            // so : (xd - xc)² + (yd - yc)² - distribute(a,b)² = 0
            // With the factor : (xd - xc)² + (yd - yc)² - (factor*distribute(a,b))² = 0
            // (xd - xc)² + (acd*xd + bcd - yc)² - (factor*distribute(a,b))² = 0
            var a = 1 + acd * acd;
